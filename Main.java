...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			intersect=foodData.getAllFoodItems();
			...
			saveFile.setPromptText("Save File Name");
			...
			bottomBox.getChildren().addAll(addNewFoodBtn, analyze, fileName, load, saveFile, save, foodQuery); //HBox for the interaction buttons and fields
			...
			save.setOnAction(e -> {
			FoodData tempFoods= new FoodData();
			for(int i=0; i< intersect.size(); i++) {
				tempFoods.addFoodItem(intersect.get(i));
			}
			tempFoods.saveFoodItems(saveFile.getText());
			
			}); //Loads the file with food items
			...
			analyze.setOnAction(e -> { //analyzes the food items in meal list and displays the meal information
				...
				for (int i = 0; i < mealList.getItems().size(); i++) {
					...
					FoodItem thisFood= new FoodItem();
					List<FoodItem> thisFoodList = foodData.filterByName(mealList.getItems().get(i));
					for( int j=0; j< thisFoodList.size(); j++) {
						if(thisFoodList.get(j).getName().equalsIgnoreCase(mealList.getItems().get(i))) {
							thisFood = thisFoodList.get(j);
						}
					}
			...
			queryEnter.setOnAction(event -> {
				...
				intersect = nutrientsTemp.stream().filter(nameTemp::contains)
